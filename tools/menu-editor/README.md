...
- **中文字体现场取模**：扫描工程全部文本（含 printf 格式串/开关文案/额外字符）+ ASCII 基线，从所选 u8g2 字体中仅抽取用到的字形，生成精简 `menu_font[]` 数组（典型中文菜单 1~3KB，替代 67KB+ 整包 GB2312 字库）；预览直接用子集字体渲染（与全字库逐像素一致），设置页可配额外包含字符

...
- **回调函数管理**：「资源」页自动收集按钮附加值回调与画板回调（按名字聚合），点击展开可统一改名（同步全部引用处）、查看 C 函数签名、点击引用条目定位到属性面板
...
- **中文字体现场取模**：扫描工程全部文本（含 printf 格式串/开关文案/额外字符）+ ASCII 基线，从所选 u8g2 字体中仅抽取用到的字形，生成精简 `menu_font[]` 数组（典型中文菜单 1~3KB，替代 67KB+ 整包 GB2312 字库）；预览直接用子集字体渲染（与全字库逐像素一致），设置页可配额外包含字符。限制（生成时给出警告）：字形数超过 255 时字体头 `glyph_cnt` 字段回绕；子集超过 16KB 时预览回退全字库（导出数组不受影响）；字符不在所选源字体中时无法收录
- **一键生成 C 代码**：单文件 `menu_pages.c`（页面函数前置声明 + main.c 所需符号的 extern 速查注释，无需 .h），CubeMX 风格 `USER CODE BEGIN/END` 保留区，再次生成不覆盖手写回调；图表填充、自绘板、按钮回调均生成骨架
...
- 变量：全部来自变量池（`Project.variables`），条目按 id 引用；旧版工程（条目自带 varName/varType）导入时自动迁移。滑块/进度条要求整型变量、开关要求 uint8，不满足时跳过绑定并给出警告。定义集中在「变量定义」区，main.c 里 `extern` 即可使用（文件头注释有速查）

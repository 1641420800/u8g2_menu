...
`,warnings:t}}function bt(i){return{raw:i.slice(0,23),glyphCnt:i[0],startUpperA:i[17]<<8|i[18],startLowerA:i[19]<<8|i[20],startUnicode:i[21]<<8|i[22]}}function Ue(i,n,e){const t=[],s=[],r=[],a=[...n].sort((g,y)=>g-y);for(const g of a){const y=e(g);if(!y||!y.length){r.push(g);continue}g<=255?t.push({encoding:g,entry:y}):s.push({encoding:g,entry:y})}if(!t.length&&!s.length)return null;const o=t.length+s.length;let u=0;for(const g of t)u+=g.entry.length;u+=2;let l=4;for(const g of s)l+=g.entry.length;l+=2;const d=bt(i),p=new Uint8Array(23+u+l);p.set(d.raw,0),p[0]=o,p[17]=0,p[18]=0,p[19]=0,p[20]=0,p[21]=0,p[22]=0;let f=23;for(const g of t){if(g.encoding===65){const y=f-23;p[17]=y>>8&255,p[18]=y&255}if(g.encoding===97){const y=f-23;p[19]=y>>8&255,p[20]=y&255}p.set(g.entry,f),f+=g.entry.length}p[f]=0,p[f+1]=0,f+=2;const h=f-23;p[21]=h>>8&255,p[22]=h&255,p[f]=0,p[f+1]=4,p[f+2]=255,p[f+3]=255,f+=4;for(const g of s)p.set(g.entry,f),f+=g.entry.length;return p[f]=0,p[f+1]=0,{font:p,included:o,missing:r}}function gt(i){return[...i].map(n=>n.codePointAt(0)).filter(n=>Number.isFinite(n))}function vt(){const i=[];for(let n=32;n<=126;n++)i.push(n);return i}function Me(i,n){const e=new Set(vt()),t=s=>{for(const r of gt(s))e.add(r)};for(const s of i.pages)for(const r of s.items)r.kind==="text"&&t(r.text),r.kind==="textarea"&&t(r.content),r.bind.type==="switch"&&(t(r.bind.onText),t(r.bind.offText));return t(n),e.delete(10),e.delete(13),e}function Fe(i){let n=0,e=0;for(const t of i)t<=126?n++:e++;return{total:i.size,ascii:n,cjk:e}}var j=(i=>(i[i.None=0]="None",i[i.Up=1]="Up",i[i.Down=2]="Down",i[i.Enter=3]="Enter",i[i.Return=4]="Return",i[i.Add=5]="Add",i[i.Sub=6]="Sub",i))(j||{});const xt=64,$t=8192/8;function _t(i){return new Promise((n,e)=>{const t=document.createElement("script");t.src=i,t.onload=()=>n(),t.onerror=()=>e(new Error(`预览引擎脚本加载失败: ${i}`)),document.head.appendChild(t)})}class yt{constructor(n,e={}){this.mod=null,this.img=null,this.raf=0,this.lastT=0,this.running=!1,this.fontIndexCache=new Map,this.structSig="",this.fontSig=null,this.lastKnownPage=0,this.fontApplyWarning=null,this.canvas=document.createElement("canvas"),this.canvas.width=128,this.canvas.height=64,this.canvas.className="ume-preview-canvas",n.appendChild(this.canvas),this.ctx=this.canvas.getContext("2d"),this.events=e}async load(n){if(this.mod)return;const e=window;e.U8G2MenuPreview||await _t(n);const t=e.U8G2MenuPreview;if(!t)throw new Error("U8G2MenuPreview 未找到（检查 wasmUrl）");this.mod=await t({locateFile:r=>n.replace(/[^/\\]*$/,"")+r}),this.mod.ccall("em_init",null,["number","number"],[128,64]);const s=this.mod._em_font_count_export();for(let r=0;r<s;r++){const a=this.mod._em_font_name(r);this.fontIndexCache.set(this.mod.UTF8ToString(a),r)}this.start()}get ready(){return!!this.mod}get currentPage(){return this.lastKnownPage}fontIndex(n){return this.fontIndexCache.get(n)??0}signature(n){return JSON.stringify({bufs:(n.chartBuffers??[]).map(e=>`${e.name}|${e.dataLen}|${e.sample}`),pages:n.pages.map(e=>({n:e.items.length,k:e.items.map(t=>t.kind).join(","),res:e.items.map(t=>t.kind==="chart"?(t.sources??[]).map(s=>`${s.bufferId}|${s.chartKind}|${s.min??"a"}|${s.max??"a"}`).join(">"):t.kind==="xbm"?`${t.w}x${t.h}`:t.kind==="textarea"?Math.ceil(t.content.length/64):"").join(",")}))})}sync(n){const e=this.mod;if(!e)return;const t=this.signature(n);t!==this.structSig&&(e.ccall("em_reset_dynamic",null,[],[]),this.structSig=t);const s=d=>Math.trunc(Number.isFinite(d)?d:0),r={uint8:0,uint16:1,uint32:2,int8:3,int16:4,int32:5,int:6,float:7,double:8},a={text:0,slider:1,progress:2,chart:3,xbm:4,textarea:5,board:6},o={none:0,value:1,switch:2,button:3,submenu:4,back:5},u={sine:0,ramp:1,noise:2,none:3},l=d=>d?(n.variables??[]).findIndex(p=>p.id===d):-1;if((n.variables??[]).forEach((d,p)=>{e.ccall("em_var_define",null,["number","number","number","number","number","number"],[p,r[d.type],s(d.initialValue),s(d.step),s(d.min),s(d.max)])}),(n.chartBuffers??[]).forEach((d,p)=>{e.ccall("em_buf_define",null,["number","number","number"],[p,s(d.dataLen),u[d.sample]])}),n.pages.forEach((d,p)=>{e.ccall("em_page_begin",null,["number"],[p]),d.items.forEach((f,h)=>{const g=()=>{const y=f.bind;if(y.type==="none")return;const v=y.type==="value"||y.type==="switch",b=v?(n.variables??[]).find(C=>C.id===y.varId):void 0;e.ccall("em_page_bind",null,["number","number","number","number","number","number","number","number"],[p,h,o[y.type],v&&b?r[b.type]:0,y.type==="switch"?s(y.openValue):0,y.type==="button"?s(y.buttonId):0,y.type==="submenu"?n.pages.findIndex(C=>C.id===y.targetPageId):-1,v&&b?l(b.id):-1]),y.type==="switch"&&e.ccall("em_item_switch_text",null,["number","number","string","string"],[p,h,y.onText,y.offText])};switch(f.kind){case"text":e.ccall("em_page_item",null,["number","number","number","number","number","number","number","number","number"],[p,h,a.text,f.scale,0,0,0,f.displayVarId?l(f.displayVarId):-1,-1]),e.ccall("em_item_text",null,["number","number","string"],[p,h,f.text]),g();break;case"slider":case"progress":e.ccall("em_page_item",null,["number","number","number","number","number","number","number","number","number"],[p,h,a[f.kind],1,0,0,0,-1,-1]),g();break;case"chart":e.ccall("em_page_item",null,["number","number","number","number","number","number","number","number","number"],[p,h,a.chart,1,s(f.height),0,0,-1,-1]);for(const y of f.sources??[])e.ccall("em_item_chart_add",null,["number","number","number","number","number","number","number"],[p,h,(n.chartBuffers??[]).findIndex(v=>v.id===y.bufferId),{line:0,point:1,bar:2}[y.chartKind],y.min!==void 0&&y.max!==void 0?1:0,y.max??0,y.min??0]);g();break;case"xbm":e.ccall("em_page_item",null,["number","number","number","number","number","number","number","number","number"],[p,h,a.xbm,1,0,s(f.w),s(f.h),-1,-1]);{const y=e._em_scratch(f.bits.length);y&&(e.HEAPU8.set(new Uint8Array(f.bits),y),e._em_item_bits(p,h,y,f.bits.length))}g();break;case"textarea":e.ccall("em_page_item",null,["number","number","number","number","number","number","number","number","number"],[p,h,a.textarea,1,s(f.height),0,0,-1,-1]),e.ccall("em_item_text",null,["number","number","string"],[p,h,f.content]),g();break;case"board":e.ccall("em_page_item",null,["number","number","number","number","number","number","number","number","number"],[p,h,a.board,1,0,s(f.w),s(f.h),-1,-1]),g();break}}),e.ccall("em_page_end",null,["number","number"],[p,d.items.length])}),e.ccall("em_pages_commit",null,["number"],[n.pages.length]),e.ccall("em_set_style",null,["number","number","number","number","number","number","number"],[this.fontIndex(n.font),{default:0,rotundity:1,square:2}[n.selector],s(n.selectorLeftMargin),s(n.selectorTopMargin),s(n.selectorLineSpacing),n.marqueeSpeed,n.marqueeHeaderLen]),n.fontSubset){const d=Me(n,n.fontExtra),p=n.font+"|"+[...d].sort((f,h)=>f-h).join(",");p!==this.fontSig&&(this.fontSig=p,this.applyFontSubset(this.fontIndex(n.font),d))}else this.fontSig!==null&&(this.fontSig=null,this.fontApplyWarning=null)}applyFontSubset(n,e){this.fontApplyWarning=null;const t=this.getFontBytes(n),s=this.glyphFetcher(n),r=t&&s?Ue(t,e,s):null;if(!r){this.fontApplyWarning="现场取模未命中任何字形，预览使用内置字体";return}this.useCustomFont(r.font)||(this.fontApplyWarning=`子集字体 ${r.font.length} 字节超过预览槽位容量，预览已回退全字库（导出的 menu_font 数组不受影响）`)}start(){if(this.running)return;this.running=!0,this.lastT=performance.now();const n=e=>{if(!this.running)return;const t=Math.min(100,Math.round(e-this.lastT));this.lastT=e,this.renderFrame(t),this.raf=requestAnimationFrame(n)};this.raf=requestAnimationFrame(n)}stop(){this.running=!1,cancelAnimationFrame(this.raf)}renderFrame(n){var o,u;const e=this.mod;if(!e)return;const t=e._em_frame(n);if(!t)return;this.img||(this.img=this.ctx.createImageData(128,64));const s=e.HEAPU8.subarray(t,t+$t),r=this.img.data;r.fill(255);for(let l=0;l<64;l++){const d=(l>>3)*128,p=1<<(l&7);let f=l*128*4;for(let h=0;h<128;h++)s[d+h]&p&&(r[f]=17,r[f+1]=24,r[f+2]=39),f+=4}this.ctx.putImageData(this.img,0,0);const a=e._em_get_current_page();a!==this.lastKnownPage&&(this.lastKnownPage=a,(u=(o=this.events).onPageChanged)==null||u.call(o,a))}key(n){var e;(e=this.mod)==null||e.ccall("em_key",null,["number"],[n])}navTo(n){var e;(e=this.mod)==null||e.ccall("em_nav",null,["number"],[n])}getFontBytes(n){const e=this.mod;if(!e)return null;const t=e.ccall("em_font_data","number",["number"],[n]),s=e.ccall("em_font_data_len","number",["number"],[n]);return!t||!s?null:e.HEAPU8.slice(t,t+s)}glyphFetcher(n){const e=this.mod;return e?t=>{const s=e.ccall("em_scratch","number",["number"],[64]),r=e.ccall("em_font_glyph","number",["number","number","number","number"],[n,t,64,s]);return r?e.HEAPU8.slice(s,s+r):null}:null}useCustomFont(n){const e=this.mod;if(!e)return!1;const t=e.ccall("em_custom_font_ptr","number",[],[]),s=e.ccall("em_custom_font_max","number",[],[]);return n.length>s?!1:(e.HEAPU8.set(n,t),e.ccall("em_set_custom_font",null,["number"],[n.length]),!0)}getInt(n){var e;return((e=this.mod)==null?void 0:e._em_get_ipool(n))??0}getSwitch(n){var e;return((e=this.mod)==null?void 0:e._em_get_upool(n))??0}destroy(){this.stop(),this.canvas.remove(),this.mod=null}}const wt=Object.keys(he);function kt(i,n,e,t){const s=i.getState(),r=e.label||"text"in e&&e.text||he[e.kind],a=e.bind.type!=="none"?` · ${pe[e.bind.type]}`:"",o=u=>l=>{l.stopPropagation(),i.getState().moveItem(n.id,e.id,u)};return m.html`<div class="ume-item-row ${t?"selected":""}"
    ...
    ${wt.map(t=>m.html`<option value=${t}>${he[t]}</option>`)}
  ...
  `}function we(i,n){return m.html`<div class="ume-field"><label></label>
    ...
    （在「资源」页修改变量）
  </div>`:m.html`${m.nothing}`}function Ct(i,n,e){const{project:t,selection:s}=n.getState(),r=t.pages.find(h=>h.id===s.pageId)??null,a=(r==null?void 0:r.items.find(h=>h.id===s.itemId))??null,o=t.variables??[],u=t.chartBuffers??[],l=(h,g)=>n.getState().updateItem(r.id,a.id,h,g),d=h=>l({bind:h});let p=m.html`<div class="ume-empty-hint">在左侧选择页面或条目</div>`,f="";if(r&&!a)f="页面属性",p=m.html`
      ${H("名称",r.name,h=>n.getState().updatePage(r.id,{name:h}))}
      ${H("C 函数名",r.fnName,h=>n.getState().updatePage(r.id,{fnName:h}),"留空自动 page_N")}
      <div class="ume-hint">页面内的手写 C 代码请到生成的 menu_pages.c 的 USER CODE 区填写（重新生成时保留），编辑器不提供代码编辑</div>
    `;else if(r&&a){(a.bind.type==="value"||a.bind.type==="switch")&&a.bind.varId&&(_e=a.bind.varId),f=`${he[a.kind]}${a.bind.type!=="none"?` + ${pe[a.bind.type]}`:""}`;let h=m.html``;switch(a.kind){case"text":{const v=a,b=o.find(C=>C.id===v.displayVarId);h=m.html`
    ...
    ${p}
  `,i)}let de=null,Ie=null;const Tt=[{value:"uint8",label:"uint8 (0~255)"},{value:"int8",label:"int8 (-128~127)"},{value:"uint16",label:"uint16 (0~65535)"},{value:"int16",label:"int16 (-32768~32767)"},{value:"uint32",label:"uint32"},{value:"int32",label:"int32"},{value:"int",label:"int"},{value:"float",label:"float (小数)"},{value:"double",label:"double (小数)"}];function At(i,n){const e=n.variables??[],t=r=>{de=de===r?null:r},s=r=>{const a=de===r.id,o=(f,h)=>i.getState().updateVariable(r.id,f,h),u=r.name&&!/^[A-Za-z_][A-Za-z0-9_]*$/.test(r.name),l=r.name&&!u&&!De(r.name),d=e.filter(f=>f.name===r.name).length>1,p=Nt(n,r.id);return m.html`<div class="ume-var-item ${a?"editing":""}">
      ...
      ${a?m.html`<div class="ume-var-edit">
        ...
        ${Z("类型",r.type,Tt,f=>o({type:f}))}
        ${B("初始值",r.initialValue,f=>o({initialValue:f},`vi-${r.id}`),"any")}
        ${B("最小值",r.min,f=>o({min:f},`vmin-${r.id}`),"any")}
        ${B("最大值",r.max,f=>o({max:f},`vmax-${r.id}`),"any")}
        ${B("步长",r.step,f=>o({step:f},`vs-${r.id}`),"any")}
  ...
  `;function A(){f.remove(),document.removeEventListener("pointerup",R)}document.addEventListener("pointerup",R),O(),i.appendChild(f)}function Ne(i,n,e){return Number.isFinite(i)?Math.min(e,Math.max(n,Math.trunc(i))):n}const Ft="prebuilt/u8g2-menu-preview.js";class jt{constructor(n,e={}){var f;if(this.store=Ge(),this.renderScheduled=!1,this.lastExport=null,this.destroyed=!1,this.activateRightTab=()=>{},this.container=n,this.opts={persistKey:"default",...e},n.classList.add("ume"),!document.getElementById("ume-style")){const h=document.createElement("style");h.id="ume-style",h.textContent=ze,document.head.appendChild(h)}const t=this.opts.persistKey?localStorage.getItem(`ume_autosave_${this.opts.persistKey}`):null,s=this.opts.data??t??void 0;if(s!==void 0)try{this.store.setState({project:$e(s)})}catch(h){console.warn("[u8g2-menu-editor] 初始数据无效，使用示例工程:",h)}const r=this.opts.persistKey?localStorage.getItem(`ume_last_c_${this.opts.persistKey}`):null;r&&(this.lastExport={c:r}),n.innerHTML=`
